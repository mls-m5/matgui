...
// data
void renderText(const FontType *font, GLubyte r, GLubyte g, GLubyte b, double x,
                double y, double z, const std::string &text, bool centered);
...
public:
    ...
    void render(FontView &view, double x, double y) {
        ...
        if (view._needsUpdate) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, internal_format, messageSurface->w,
                         messageSurface->h, 0, GL_RGBA, GL_UNSIGNED_BYTE,
        ...
        drawTextureRect({x, y}, 0, view._width, view._height, texture,
                        centered ? DrawStyle::CenterOrigo
...
void renderText(const FontType *font, GLubyte r, GLubyte g, GLubyte b, double x,
    ...
    drawTextureRect({x, y}, 0, messageSurface->w, messageSurface->h, Texture,
                    centered ? DrawStyle::CenterOrigo
                             : DrawStyle::OrigoTopLeft);
