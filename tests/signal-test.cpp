...
TEST_CASE("std::bind") {
	...
	std::function<void()> function = std::bind(&VoidCallbackClass::voidCallback, &testClass);
	signal.connect(function);
	signal.emit();
	...
	ASSERT(not testClass.isCalled, "callback was called but should be disconnected");
}
...
TEST_CASE("standard function") {
	...
	signal.emit();
	flushSignals();
	ASSERT(not functionIsCalled, "function is called but should be disconnected")
}
